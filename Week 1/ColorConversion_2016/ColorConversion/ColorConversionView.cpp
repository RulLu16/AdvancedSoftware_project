...
void CColorConversionView::OnProblem1Sub2()
{
	...
	for (int i = 0; i < height * width; i++) {
		...
		else mg_lpImage[i] = m_dibFile.m_lpImage[i] * 2 ;
	}
...
void CColorConversionView::OnProblem2()
{
	...
	for (int i = 0; i < height * width * 3; i+=3) {
		char myb = m_dibFile.m_lpImage[i] * 0.114;
		char myg = m_dibFile.m_lpImage[i + 1] * 0.587;
		char myr = m_dibFile.m_lpImage[i + 2] * 0.299;

		mg_lpImage[i/3] = myb + myg + myr;
	}
...
void CColorConversionView::OnProblem3()
{
	...
	for (int i = 0; i < height * width; i++) {
		rd = (tempim16[i] & 0x7C00) /1024;
		gr = (tempim16[i] & 0x03E0)/32;
		bl = tempim16[i] & 0x001F;
