...
__global__ void Sum_n_elements_Kernel_shared(IN int *d_ArrayElements, OUT int *d_SumOfArrayElements, int N, int Nf) {
	...
	} // threadidx == 0 인 경우 예외처리
	if (thread_id == BLOCK_SIZE - 1) {
		for (int i = 0; i < Nf; i++) {
			if (id + i >= N - 1) shared_buffer[Nf + BLOCK_SIZE + i] = 0;
			else shared_buffer[Nf + BLOCK_SIZE + i] = d_ArrayElements[id + i + 1];
		}
	}
	...
	int sum = 0;
	for (int i = -Nf; i <= Nf; i++) {
		//if (thread_id + i >= N || thread_id + i < 0) continue;
