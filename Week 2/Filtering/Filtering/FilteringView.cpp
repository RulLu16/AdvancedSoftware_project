		...
		for (c=0; c<m_width; c++) {
			...
			for (int i = r - w0 / 2; i <= r + w0 / 2; i++) {
				for (int j = c - w0 / 2; j <= c + w0 / 2; j++) {
					...
					meanX += m_imageBits[i * m_width + j] * Sx[idx/3][idx%3];
					meanY += m_imageBits[i * m_width + j] * Sy[idx / 3][idx % 3];

					idx++;
				}
			}

			G[r*m_width+c] = sqrt((meanX*meanX) + (meanY*meanY));
		}
	}
...
void CFilteringView::AverageFiltering(int w0)
{
	...
	for (r=0; r<m_height; r++) {
		for (c=0; c<m_width; c++) {
			...
			for (int i = r-w0/2; i <=r+w0/2; i++) {
				for (int j = c- w0 / 2; j <= c+ w0 / 2; j++) {
					if (i < 0 || i >= m_height || j < 0 || j >= m_width) continue;

					mean += imageBits[i * m_width + j];
				}
			}

			imageBitsFiltered[r*m_width+c] = mean/(w0*w0);
		}
	}
	QueryPerformanceCounter(&m_end);
	m_result = 1000*(m_end.QuadPart-m_start.QuadPart)/m_frequency.QuadPart;	
	...
	for (r=0; r<m_height; r++) {
		...
		for (int i = r - w0 / 2; i <= r + w0 / 2; i++) {
			for (int j = c - w0 / 2; j <= c + w0 / 2; j++) {
				...
				mean += m_imageBits[i * m_width + j];
			}
		}
		m_imageBitsFiltered[r * m_width + c] = mean / (w0 * w0);
		...
		for (c=1; c<m_width; c++) {
			...
			for (int i = r - w0 / 2; i <= r + w0 / 2; i++) {
				...
				if ((c-1) - w0 / 2 >= 0)
					mean -= m_imageBits[i * m_width + ((c-1) - w0 / 2)];
				if (c + w0 / 2 < m_width)
					mean += m_imageBits[i * m_width + (c + w0 / 2)];
			}
			...
			for (int i = r - w0 / 2; i <= r + w0 / 2; i++) {
				for (int j = c - w0 / 2; j <= c + w0 / 2; j++) {
					if (i < 0 || i >= m_height || j < 0 || j >= m_width) {
						...
						medarray[idx] = m_imageBits[i * m_width + j];
					}
					idx++;
				}
			}
