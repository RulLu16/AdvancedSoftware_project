...
#define NF_SIZE (1 << 7)													// Nf 크기

#define BLOCK_SIZE (1 << 5)													// CUDA 커널 thread block 사이즈
...
cudaError_t Sum_n_elements_GPU(IN int *p_ArrayElements, OUT int *p_SumOfElements_GPU, int Nf) {
    ...
    CUDA_CALL(cudaMalloc(&copy_e, size));
    CUDA_CALL(cudaMemcpy(copy_e, p_ArrayElements, size, cudaMemcpyHostToDevice));
    CUDA_CALL(cudaMalloc(&copy_s, size));
    ...
    Sum_n_elements_Kernel << < dimGrid, dimBlock >> > (copy_e, copy_s, N, Nf);
    ...
    CUDA_CALL(cudaMemcpy(p_SumOfElements_GPU, copy_s, size, cudaMemcpyDeviceToHost));
